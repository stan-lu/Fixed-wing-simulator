...
function [delta, x_command] = autopilot_uavbook(Va_c,h_c,chi_c,Va,h,chi,phi,theta,beta,p,q,r,t,P)
    ...
    if t==0
        % 假设没有舵，因此设置delta_r=0
        delta_r = slide_hold(beta, 1, P);
        phi_c   = course_hold(chi_c, chi, r, 1, P);
        delta_a = roll_hold(phi_c, phi, p, 1, P);
    else
        phi_c   = course_hold(chi_c, chi, r, 0, P);
        delta_a = roll_hold(phi_c, phi, p, 0, P);
        delta_r = slide_hold(beta, 0, P);
    end
...
function [delta, x_command] = autopilot_TECS(Va_c,h_c,chi_c,Va,h,chi,phi,theta,p,q,r,t,P)
    ...
    if t==0
        ...
        phi_c   = course_hold(chi_c, chi, r, 0, 0,P);
        delta_r = 0;
    end
...
function delta_r = slide_hold(beta, flag, P)
    persistent integrator;
    persistent error_d1;
    if flag==1       % reset (initialize) persistent variables when flag==1
        integrator = 0;
    ...
    if P.ki_beta~=0        % implement integrator anti-windup
        ...
        integrator = integrator + P.Ts / P.ki_beta * (0 - u_unsat);
    end
end
