...
if ( (norm(start_node(1:3) - end_node(1:3)) < segmentLength )...
    ...
    numPaths = 0;
    while numPaths<3 % num of path generated
...
flag1 = 0;
% add collison free new_node to tree
while flag1==0
    % select a random point
    randomNode=generateRandomNode(map,pd,0);
    ...
    dire = new_point(1:2) - tree(idx,1:2);
    dire = dire / norm(dire);
    theta = acos(dot(dire, [0; 1]));
    cro = cross([0, 1, 0], [dire, 0]);
    chi = theta * -sign(cro(3));
...
while ptr <= size(path,1)-1
    if collision(newPath(end,:), path(ptr+1,:), map)~=0 % if there is a collision
        dire = path(ptr,1:2) - newPath(end,1:2);
        dire = dire / norm(dire);
        theta = acos(dot(dire, [0; 1]));
        cro = cross([0, 1, 0], [dire, 0]);
        chi_new = theta * -sign(cro(3));
        newPath = [newPath; path(ptr,1:3), chi_new,path(ptr,5:7)];  % add previous node
    end
    ptr = ptr+1;
end
