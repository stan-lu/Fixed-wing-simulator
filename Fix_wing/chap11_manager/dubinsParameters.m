...
%     e_se=pe-ps;
    % compute L1
    theta =angle(cre-crs);
    L1 = norm(cre-crs)+R*mod(2*pi+mod(theta-pi/2,2*pi)-mod(chis-pi/2,2*pi),2*pi)+...
    ...
    % compute L4
    theta = angle(cle-cls); 
    L4 = norm(cle-cls)+R*mod(2*pi+mod(chis+pi/2,2*pi)-mod(theta+pi/2,2*pi),2*pi)+...
