...
%     r        = uu(12+NN);      % yaw rate    
    ...
    if t==0
        ...
        hold on
        trajectory_handle = plot3(pe,pn,-pd,"k","LineWidth",1.5);
        hold on
...
%         drawPath(path, S, path_handle);
        set(trajectory_handle,'Xdata',[get(trajectory_handle,'Xdata'),pe]);
        set(trajectory_handle,'Ydata',[get(trajectory_handle,'Ydata'),pn]);
        set(trajectory_handle,'Zdata',[get(trajectory_handle,'Zdata'),-pd]);
...
function handle = drawWaypoints(waypoints, R_min, handle, mode)
    ...
    if isempty(handle)
        handle = plot3(YY,XX,-ZZ,'m--', 'LineWidth', 1.5);
    else
        set(handle,'XData', YY, 'YData', XX, 'ZData', -ZZ);
        drawnow
    end
