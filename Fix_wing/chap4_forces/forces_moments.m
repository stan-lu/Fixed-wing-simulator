...
% x 状态；delta 操纵面角度
    ...
    V_wind_body = rotation_to_body * [w_ns; w_es; w_ds] + [u_wg; v_wg; w_wg];%机体坐标系上的风速（稳定风+干扰风）
    V_airspeed_body = [u-V_wind_body(1); v-V_wind_body(2); w-V_wind_body(3)];%机体坐标系上的空速, 即相对于飞机周围空气速度
    
    % 计算北东地坐标系风数据, 即相对惯性系风速
    V_w = rotation_to_body'*V_wind_body;
    w_n = V_w(1);
    w_e = V_w(2);
    w_d = V_w(3);
%     w_n = 0;
...
%     w_d = 0;
    ...
    Va = sqrt(u_r^2 + v_r^2 + w_r^2);     %空速
    alpha = atan(w_r / u_r);  %攻角
    beta = asin(v_r / (Va));   % 侧滑角
